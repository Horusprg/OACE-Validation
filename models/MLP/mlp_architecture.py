...
class MLP(nn.Module):
    ...
    def __init__(
        ...
        if not isinstance(hidden_layers, list) or not all(
            isinstance(x, int) for x in hidden_layers
        ):
        ...
        layers.append(MLPOutputLayer(in_features=current_dim, out_features=output_dim))
...
def generate_mlp_architecture(
    ...
    if min_neurons_per_layer > max_neurons_per_layer:
        raise ValueError(
            "min_neurons_per_layer deve ser menor que max_neurons_per_layer"
        )
    ...
    if activation_fn is None:
        raise ValueError(
            f"Função de ativação '{activation_function_choice}' não suportada"
        )

    # Gera as dimensões das camadas ocultas
    hidden_layers_dims = [
        torch.randint(
            low=min_neurons_per_layer, high=max_neurons_per_layer + 1, size=(1,)
        ).item()
