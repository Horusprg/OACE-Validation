...
class DepthwiseSeparableConv(nn.Module):
    ...
    """
    def __init__(self, in_channels, out_channels, stride=1, activation_fn=nn.ReLU, dropout_rate=0.0):
        super().__init__()
...
class MobileNet(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        # Configuração padrão dos blocos (MobileNet v1)
        ...
        # Ajusta canais pelo width_multiplier
        ...
        for out_channels, stride in conv_configs:
            layers.append(
                DepthwiseSeparableConv(
                    ...
                    activation_fn=nn.ReLU, dropout_rate=dropout_rate
                )
            )
...
def generate_mobilenet_architecture(
    ...
    """
    if randomize:
        width_multiplier = width_multiplier if width_multiplier is not None else random.choice([0.5, 0.75, 1.0, 1.25])
        resolution_multiplier = resolution_multiplier if resolution_multiplier is not None else random.choice([0.5, 0.75, 1.0])
        dropout_rate = dropout_rate if dropout_rate is not None else random.choice([0.0, 0.1, 0.2, 0.3])
    else:
    ...
    print(f"MobileNet: width_multiplier={width_multiplier}, resolution_multiplier={resolution_multiplier}, dropout={dropout_rate}")
    return MobileNet(
...
if __name__ == "__main__":
    # Demonstração de geração e forward pass
